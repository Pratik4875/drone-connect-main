...
import { getCustomerBookings } from "@/api/user_api"; // API function
...
const Page = () => {
  ...
  const {
    ...
    queryKey: ["bookings", activeTab, debouncedSearchTerm, page], // Cache keys
    queryFn: () =>
      getCustomerBookings({
        status: activeTab,
      ...
      <div className="bg-white shadow-xl my-10 max-w-4xl mx-auto rounded-xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-blue-500 px-6 py-8">
          ...
          <p className="text-indigo-100 mt-2">
            Manage booking requests and pilot assignments
          </p>
        </div>
                  ...
                  {bookings.length > 0 ? (
                    ...
                    bookings.map((booking: any) => (
                      <BookingCard
                        ...
                        companyName={booking.companyInfo?.name}
                        customerName={booking.customerInfo?.name} // For pilot & company views
                      />
                    ))
                  ) : (
...
export default withAuth(Page, ["c"]);
